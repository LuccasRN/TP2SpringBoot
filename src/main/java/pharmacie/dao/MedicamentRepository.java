...
public interface MedicamentRepository extends JpaRepository<Medicament, Integer> {
    ...
    @Query("SELECT m FROM Medicament m WHERE m.categorie.code = :catId AND m.indisponible = false AND m.unitesEnStock >= m.unitesCommandees")
    List<Medicament> findDisponiblesByCategorie(@Param("catId") Integer categorieId);
}
